...
<head>
    ...
    <link rel="stylesheet" href="{{asset("css/adminExterior.css")}}">
    @yield("stylelink")
...
{{--            <span class="logo-text"><span>Shipex</span></span>--}}
        ...
        <nav class="navbar-menu">
            <ul>
                <li><a href="{{route('indexAdmin')}}" class="">Applications</a></li>
                <li>
                    <a href="{{route('adminTrackDeliveriesView')}}" class="">Deliveries</a>
                </li>
                <li>
                    <a href="{{route('AdminCitiesView')}}">Cities</a>
                </li>
                <li><a href="{{route('AdminReportsView')}}">Reports</a></li>
                @if(Auth::user()->id == 3)
                <li><a href="{{ route('support.chat') }}" target="_blank">Support</a></li>
                @endif
            </ul>
        </nav>
        ...
        @if(Auth::check())
            ...
            <a style="color: white;" href="{{route('displayAdminProfile', ["id" => Auth::user()->id])}}"> Welcome, <b class="userName">{{ Auth::user()->userName }}</b></a>
            </div>
        @else
...
@yield("content")
<!-- footer -->
<footer class="site-footer pbmit-bg-color-secondary" style="margin-top: 6%">
...
</body><script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>
